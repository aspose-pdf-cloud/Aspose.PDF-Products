...
{{% blocks/products/pf/agp/feature-section-col title="Steps to update images in PDF documents via Cloud Node.js SDK" %}}
...
describe("images Tests", () => {
    ...
    describe("GetDocumentimages Test", () => {

        it("should return response with code 200", async () => {

            return BaseTest.getPdfApi().getDocumentimages(name, null, BaseTest.remoteTempFolder)
                .then((result) => {
                    assert.equal(result.response.statusCode, 200);
            });
        });
